# beowulf_bootstrap_v2.py  (v2.1)
# Generates the entire Beowulf Streamlit app into ./beowulf_app and writes requirements/config.
...
def w(path: Path, s: str):
    """Write dedented text to a file with UTF-8 encoding."""
...
# Ensure absolute imports (when Streamlit runs file inside package dir)
...
# Do NOT call st.set_option('server.maxUploadSize'): Streamlit forbids runtime changes.
inject_ios_theme()
...
w(app/"config.py", """
import os
PROFILE = os.environ.get("BEOWULF_PROFILE", "mobile").strip().lower()
IS_MOBILE = PROFILE == "mobile"
# Read upload limit from env (recommended). Default 1 GB on mobile, 32 GB on desktop.
UPLOAD_MB = int(os.environ.get("STREAMLIT_SERVER_MAX_UPLOAD_SIZE", 1024 if IS_MOBILE else 32768))
...
def read_with_labels(file_or_path, file_name: Optional[str] = None) -> Tuple[pd.DataFrame, dict]:
    ...
    df, meta = (pyreadstat.read_sav if ext==".sav" else pyreadstat.read_dta if ext==".dta" else pyreadstat.read_sas7bdat)(file_or_path, apply_value_formats=False)
...
def _bytes_like(obj) -> bool: return hasattr(obj,"read") or isinstance(obj,(bytes,bytearray,io.BytesIO))
